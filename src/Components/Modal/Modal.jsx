...
const Modal = ({ children, isActive, onHandleClose }) => {
  return (
    <>
      <div
      ...
      ></div>
      <div
        className={`fixed top-1/2 left-1/2 transition ease-in duration-300 ${
          isActive
            ? "visible opacity-1 select-auto scale-1"
            : "opacity-0 select-none invisible scale-[0.4]"
        } -translate-x-1/2 -translate-y-1/2 h-auto w-auto bg-white z-[10000]`}
      >
        {children}
      </div>
    </>
